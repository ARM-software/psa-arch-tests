...
const client_test_t test_c069_crypto_list[] = {
    NULL,
    psa_pake_setup_test,
    psa_pake_setup_negative_test,
    NULL,
};
...
int32_t psa_pake_setup_test(caller_security_t caller __UNUSED)
{
...
int32_t psa_pake_setup_negative_test(caller_security_t caller __UNUSED)
{
    int32_t                 status;
    psa_pake_cipher_suite_t cipher_suite = PSA_PAKE_CIPHER_SUITE_INIT;
    psa_pake_operation_t    operation = PSA_PAKE_OPERATION_INIT;
    ...
    if (valid_test_input_index < 0)
      ...
      /* Setting up the watchdog timer for each check */
        ...
        val->crypto_function(VAL_CRYPTO_PAKE_CS_SET_PRIMITIVE,
                             &cipher_suite,
                            check1[valid_test_input_index].pake_primitive);
        val->crypto_function(VAL_CRYPTO_PAKE_CS_SET_KEY_CONFIRMATION,
                             &cipher_suite,
                             check1[valid_test_input_index].key_confirmation);

   val->print(PRINT_TEST, "[Check %d] Test psa_pake_setup - zero as key handle\n", g_test_count++);

       /* Setup PAKE opeartion object with invalid key */
        status = val->crypto_function(VAL_CRYPTO_PAKE_SETUP,
                                      &operation,
                                      0,
                                      &cipher_suite);
