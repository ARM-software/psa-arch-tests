...
const client_test_t test_c070_crypto_list[] = {
    NULL,
    psa_pake_set_role_test,
    NULL,
 };

extern  uint32_t g_test_count;

int32_t psa_pake_set_role_test(caller_security_t caller __UNUSED)
{
 ...
 for (i = 0; i < num_checks; i++)
    {
        ...
        /* Setup application role for password authenticated key exchange */
    ...
    /* Expect a bad state when psa_pake_set_role is called on aborted inactive operation object */
      status = val->crypto_function(VAL_CRYPTO_PAKE_SET_ROLE,
                                    &operation,
                                    check1[i].role);
